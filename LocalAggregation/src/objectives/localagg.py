...
class MemoryBank(object):
    ...
    def __init__(self, size, dim, device_ids, bank=None):
        self.size = size
        self.dim = dim
...
class Kmeans(object):
    ...
    """
    def __init__(self, k, memory_bank, gpu_device=0, fvs=None):
        super().__init__()
        self.k = k
    ...
    def compute_clusters(self):
        ...
        if self.fvs is not None and len(self.fvs) != 0:
            data = torch.cat((data.cpu().float(), self.fvs.float()), 1)
